    ...
    public partial class frmProductGroup : Form
    {
        ...
        private void dgvProductGroup_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm deletion for Product Group Code: {0}.", productGroupCode), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    e.Cancel = true;
                else
                {
