    ...
    public partial class frmImport : Form
    {
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(messageString, "Bassett Forecast");
                    if (importError)
                        return;
                }
                switch (_importFileType)
        ...
        private void txtFilePath_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(string.Format("Please enter a valid {0} file path.", _importFileType == ImportFileType.POSXLSACNeilsen ? "Excel" : "text"), "Bassett Forecast");
                    e.Cancel = true;
                }
            }
            catch (Exception ex)
