    ...
    public partial class frmPWCForecast : Form
    {
        ...
        private void frmPWCForecast_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                var rx = new System.Text.RegularExpressions.Regex(@"^[PCN]Y[01]\d$");
                foreach (var column in dgvForecast.Columns.Cast<DataGridViewColumn>().Where(column => rx.IsMatch(column.HeaderText)))
        ...
        private void RestorePriorToImport(string storedProcedureName, string tableName)
        {
            ...
            MessageBox.Show(sb.ToString(), "Bassett Forecast");
        }
        ...
        private void itemToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                if (!Validate())
                    return;
                var frmItem = new frmItem(_company);
        ...
        private void generateCompanyForecastToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_company == null)
                {
                    MessageBox.Show("Please ennter a Company Code.", "Bassett Forecast");
        ...
        private void txtCompanyCode_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please enter a 3 character Company Code.", "Bassett Forecast");
                    e.Cancel = true;
                }
                    ...
                    _company == null)
                {
                    ...
                    {
                        MessageBox.Show("Invalid Company Code.", "Bassett Forecast");
                        e.Cancel = true;
                        return;
                    }
            ...
            if (rbtnSR.Checked)
            {
                ...
                if (posDateAndGapCollection.Count > 0)
                {
                    ...
                    if (posLatestDateAndGap.GapFlag)
                        MessageBox.Show("There's gap in the POS period selected or no POS data.", "Bassett Forecast");
                }
                else
                    MessageBox.Show("There's gap in the POS period selected or no POS data.", "Bassett Forecast");
            }
            else
            {
                ...
                if (actualSalesDateAndGapCollection.Count > 0)
                {
                    ...
                    if (actualSalesDateAndGap.GapFlag)
                        MessageBox.Show("There's gap in the Actual Sales period selected or no Actual Sales data.", "Bassett Forecast");
                }
                else
                    MessageBox.Show("There's gap in the Actual Sales period selected or no Actual Sales data.", "Bassett Forecast");
            }
        ...
        private void dtpMonthEndDate_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please select a week ending Sunday.", "Bassett Forecast");
                    e.Cancel = true;
                    return;
                }
                if (txtCompanyCode.Text.Length == 0)
                {
                    MessageBox.Show("Please enter a Company Code.", "Bassett Forecast");
                    txtCompanyCode.Focus();
                    return;
                }
                if (cboCustomer.SelectedIndex < 1)
                {
                    MessageBox.Show("Please select a Customer Number.", "Bassett Forecast");
                    cboCustomer.Focus();
                    return;
                }
        ...
        private void btnGenerateSalesRate_Click(object sender, EventArgs e)
        {
            try
            {
                if (_company != null && _customerKey != null)
                {
                    ...
                    {
                        if (MessageBox.Show("A saved Sales Rate for the period exists.\nDo you want to regenerate new Sales Rate?", "Bassett Forecast", MessageBoxButtons.YesNo) == DialogResult.No)
                            return;
                    }
                    Cursor = Cursors.WaitCursor;
                    customer.ForecastSalesRateCollection = null;
            ...
            try
            {
                if (_company != null && _customerKey != null)
                {
                    ...
                    if (customer.ForecastSalesRateCollection.Count <= 0)
                        MessageBox.Show("The Sales Rate collection is empty.", "Bassett Forecast");
                    else
                    {
                        Cursor = Cursors.WaitCursor;
        ...
        private void btnGenerateForecast_Click(object sender, EventArgs e)
        {
            try
            {
                if (_company != null && _customerKey != null)
                {
                    ...
                    {
                        MessageBox.Show("Past Forecast cannot be overwritten.", "Bassett Forecast", MessageBoxButtons.OK);
                        return;
                    }
                    ...
                    {
                        if (MessageBox.Show("A saved Forecast for the period exists.\nDo you want to regenerate new Forecast?", "Bassett Forecast", MessageBoxButtons.YesNo) == DialogResult.No)
                            return;
                    }
                    Cursor = Cursors.WaitCursor;
                    customer.ForecastCollection = null;
        ...
        private void btnImportForecast_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("The number of columns in the txt file must be 41. The error is on line 2.", "PWC Forecast",
                ...
                {
                    MessageBox.Show("Forecast to be imported has different Company and Customer Codes.", "PWC Forecast",
                                    MessageBoxButtons.OK);
                    return;
                }
                ...
                if ((customer.ForecastAction == ForecastAction.Get && customer.ForecastCollection.Count > 0) ||
                    PersistenceLayer.Utility.GetInstance()
                                    .DoesForecastExist(_customerKey.CompanyCode, _customerKey.CustomerNumber,
                                                       posSalesEndDate))
                {
                    if (
                        MessageBox.Show("A saved Forecast for the period exists.\nDo you want to import new Forecast?",
                                        "PWC Forecast", MessageBoxButtons.YesNo) == DialogResult.No)
                        return;
                }
                ...
                Cursor = Cursors.WaitCursor;
                customer.ForecastAction = ForecastAction.Generate;
                customer.PopulateForecastFromCsv(osd.FileName);
                bindingSourceForecast.DataSource = customer.ForecastCollection;
                lblForecastDescription.Text =
                    string.Format("Company: {0}, Customer: {1} - Imported Forecast: (POS/Actual Sale ends {2})",
                                  _company.CompanyCode, _customerKey.CustomerNumber,
                                  customer.POSSalesEndDate.Value.ToString("MM/dd/yyyy"));
                if (customer.ForecastCollection != null && customer.ForecastCollection.Count > 0)
                    dgvForecast.Focus();
                Cursor = Cursors.Arrow;
            }
        ...
        private void dgvParameter_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                ...
                else if (rx.IsMatch(column.HeaderText)) // CY##, NY##, PY##
                {
        ...
        private void dgvParameter_CellFormatting(object sender, System.Windows.Forms.DataGridViewCellFormattingEventArgs e)
        {
            try
            {
                if (e.ColumnIndex < 0)
                    return;
                var rx = new System.Text.RegularExpressions.Regex(@"^[PCN]Y[01]\d$");
                if (rx.IsMatch(dgvParameter.Columns[e.ColumnIndex].HeaderText)) // PY##, CY##, NY## columns
                {
                    if (e.Value != null)
                    {
        ...
        private void dgvParameter_CellParsing(object sender, DataGridViewCellParsingEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    else if (rx.IsMatch(column.HeaderText)) // PY##, CY##, NY## columns
                    {
        ...
        private void dgvParameter_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                if (_company == null)
                {
                    MessageBox.Show("Please enter Company Code.", "Bassett Forecast");
                ...
                {
                    MessageBox.Show("Please enter Customer Number.", "Bassett Forecast");
                    dgvParameter.CancelEdit();
                    cboCustomer.Focus();
                    return;
                }
                ...
                switch (column.Name)
                {
                    case "ItemNumberParameter":
                        ...
                        {
                            var itemCollection = new Item.ParameteredCollection(_company.CompanyCode, formattedValue, SqlString.Null, SqlString.Null, SqlString.Null, SqlString.Null);
                            itemCollection.Load();
                    ...
                    case "StoreCountExisting":
                        rx = new System.Text.RegularExpressions.Regex(@"^\-?\d{1,6}$");
                        if (!rx.IsMatch(formattedValue))
                        {
                    ...
                    case "InitialQtyPerNewStore":
                        rx = new System.Text.RegularExpressions.Regex(@"^\d{0,5}(\.\d{1,2})?$");
                    ...
                    case "ProjectedSalesRateExisting":
                        rx = new System.Text.RegularExpressions.Regex(@"^\d{0,3}(\.\d{1,2})?$");
                        if (formattedValue == string.Empty || !rx.IsMatch(formattedValue))
                        {
                    ...
                    case "PipelineEnd":
                    case "PipelineStart":
                        rx = new System.Text.RegularExpressions.Regex(@"^[PCN]Y[01]\d$");
                        if (!rx.IsMatch(formattedValue))
                        {
                        ...
                        break;
                    default:
                        if (rx.IsMatch(column.HeaderText) &&
        ...
        private void dgvParameter_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm Forecast Parameter deletion for Item: {0}.", itemNumber), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    e.Cancel = true;
                else
                {
                    var customer = _company.CustomerCollection[_customerKey];
            ...
            try
            {
                ...
                switch (column.Name)
                {
                    ...
                    case "DiscontinuedEffectiveDate":
                        ...
                        break;
                    default:
                        if (rx.IsMatch(column.Name)) // PY##, CY##, NY## columns
                        {
                            if (e.Value.ToString() == "Null")
                            {
        ...
        private void dgvBonusAndDiscontinued_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                {
                    var formattedValue = e.FormattedValue.ToString();
                    var rx0 = new System.Text.RegularExpressions.Regex(@"^[PCN]Y[01]\d");
                    System.Text.RegularExpressions.Regex rx1;
                    var column = dgvBonusAndDiscontinued.Columns[e.ColumnIndex];
                    if (column.Name == "ItemNumberBonus")
                    {
                        rx1 = new System.Text.RegularExpressions.Regex(@"^\d{2}-\d{5}-[A-Z0-9]{2}\d$");
                        if (!rx1.IsMatch(formattedValue))
                        {
                        ...
                        formattedValue != "Null")
                    {
                        rx1 = new System.Text.RegularExpressions.Regex(@"^\d{1,2}/\d{1,2}/\d{2,4}$");
                        if (formattedValue == string.Empty || !rx1.IsMatch(formattedValue))
                        {
                    ...
                    else if (rx0.IsMatch(column.Name) &&
                        formattedValue != "Null") // PL##, CY##, NY##
                    {
                        var daysOfMonth = int.Parse(rx0.Replace(column.HeaderText, string.Empty).Replace("(", string.Empty).Replace(")", string.Empty));
                        rx1 = new System.Text.RegularExpressions.Regex(@"^\d{0,2}$");
                        if (formattedValue == string.Empty || !rx1.IsMatch(formattedValue))
                        {
        ...
        private void dgvBonusAndDiscontinued_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm Bonus & Discontinued deletion for Item: {0}.", itemNumber), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    e.Cancel = true;
                else
                {
                    if (rbtnBonusAndDiscontinuedByCustomer.Checked)
        ...
        private void dgvSalesRate_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                {
                    rx = new System.Text.RegularExpressions.Regex(@"^\d{2}-\d{5}-[A-F0-9]{2}\d$");
                    if (!rx.IsMatch(formattedValue))
                    {
                        row.ErrorText = "Please enter Item Number in ##-#####-AA";
                        e.Cancel = true;
        ...
        private void dgvSalesRate_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm Sales Factor deletion for Item: {0}.", itemNumber), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                {
                    e.Cancel = true;
                    return;
                }
        ...
        private void dgvTrend_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please select Customer Number.", "Bassett Forecast");
                    dgvTrend.CancelEdit();
                    cboCustomer.Focus();
                    return;
                }
                ...
                {
                    rx = new System.Text.RegularExpressions.Regex(@"^\d{0,3}(\.\d{1,2})?$");
                    if (formattedValue == string.Empty || formattedValue == "Null" || !rx.IsMatch(formattedValue))
                    {
        ...
        private void dgvTrend_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (rbtnTrendByCompanyProductGroup.Checked || rbtnTrendByCustomerProductGroup.Checked)
                {
                    ...
                    if (MessageBox.Show(string.Format("Please confirm Trend Factor deletion for Product Group: {0}, Forecast Method: {1}.", productGroupCode, forecastMethod), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    {
                        e.Cancel = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    if (MessageBox.Show(string.Format("Please confirm Trend Factor deletion for Item: {0}, Forecast Method: {1}.", itemNumber, forecastMethod), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    {
                        e.Cancel = true;
                        return;
                    }
        ...
        private void dgvForecast_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    var rx = new System.Text.RegularExpressions.Regex(@"^[PCN]Y[01]\d$");
                    if (rx.IsMatch(column.HeaderText)) // CY##, NY##, PY##
                    {
...
#if TEMPFORECASTITEM
                var itemCollection = new Item.ParameteredCollection(_company.CompanyCode, itemNumber, SqlString.Null, SqlString.Null, SqlString.Null, SqlString.Null);
                itemCollection.Load();
                if (itemCollection.Count == 0)
                {
                    row.ErrorText = "Item does not exist in Item Master";
        ...
        private void dgvForecast_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm Forecast deletion for Item: {0}, Forecast Method: {1}.", itemNumber, forecastMethod), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                {
                    e.Cancel = true;
                    return;
                }
        ...
        private void forecastContextPasteMenuItemClick(object sender, EventArgs e)
        {
            try
            {
                ...
                for (var i = 0; i < dgvForecast.SelectedCells.Count; i++)
                    ...
                    {
                        MessageBox.Show("Only single row selection is supported for range pasting.", "Bassett Forecast");
                        return;
                    }
