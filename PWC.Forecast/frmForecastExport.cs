    ...
    public partial class frmForecastExport : Form
    {
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please select a Saved Forecast.", "Bassett Forecast");
                    cboSavedForecast.Focus();
                    return;
                }
                if (!Directory.Exists(Path.GetDirectoryName(txtFilePath.Text)))
                {
                    MessageBox.Show("Export file folder does not exist. Please create the folder and try again.", "Bassett Forecast");
                    txtFilePath.Focus();
                    return;
                }
...
#if TEMPGROUPING
                if (forecastReportType == ForecastExportType.PipelineCrosstab)
                {
                ...
                {
                    if (rbtnByDate.Checked)
                        _company.ExportForecast(DateTime.Parse(((PWC.PersistenceLayer.Utility.ForecastDateMethod)cboSavedForecast.SelectedItem).POSSalesEndDate), txtFilePath.Text, chkAppendFile.Checked, chkSubtractCurrentMonthSales.Checked);
                    else
                        _company.ExportForecast(txtFilePath.Text, chkAppendFile.Checked, chkSubtractCurrentMonthSales.Checked);
                }
#else
                if (rbtnByDate.Checked)
                    _company.ExportForecast(DateTime.Parse(((PWC.PersistenceLayer.Utility.ForecastDateMethod)cboSavedForecast.SelectedItem).POSSalesEndDate), txtFilePath.Text, chkAppendFile.Checked, forecastReportType, chkSubtractCurrentMonthSales.Checked);
        ...
        private void txtFilePath_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please enter a valid .csv file path.", "Bassett Forecast");
                    e.Cancel = true;
                }
            }
            catch (Exception ex)
