    ...
    public partial class frmForecastVarianceReport : Form
    {
        ...
        private void btnReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please select at least one Forecast Method check box.", "Bassett Forecast");
                    chkSRRA.Focus();
                    return;
                }
                if (cboCustomerFrom.SelectedValue.ToString().CompareTo(cboCustomerTo.SelectedValue.ToString()) == 1)
                {
                    MessageBox.Show("Customer From must be less or equal Customer To.", "Bassett Forecast");
                    cboCustomerFrom.Focus();
                    return;
                }
                if (cboBrandFrom.SelectedValue.ToString().CompareTo(cboBrandTo.SelectedValue.ToString()) == 1)
                {
                    MessageBox.Show("Brand From must be less or equal Brand To.", "Bassett Forecast");
                    cboBrandFrom.Focus();
                    return;
                }
                if (cboProductCategoryFrom.SelectedValue.ToString().CompareTo(cboProductCategoryTo.SelectedValue.ToString()) == 1)
                {
                    MessageBox.Show("Product Category From must be less or equal Product Category To.", "Bassett Forecast");
        ...
        private void txtFilePath_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please enter a valid text file path.", "Bassett Forecast");
                    e.Cancel = true;
                }
            }
            catch (Exception ex)
        ...
        private void txtForecastYear_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show(string.Format("Please enter year in range of {0} - {1}.", minYear, maxYear), "Bassett Forecast");
                    e.Cancel = true;
                }
            }
            catch (Exception ex)
