...
namespace PWC.Forecast
{
    public partial class frmItem : Form
    {
        ...
        public frmItem(Company company)
        {
            InitializeComponent();
            _company = company;
            if (company != null)
        ...
        private void dgvItem_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show(string.Format("Please confirm deletion for Item Number: {0}.", itemNumber), "Bassett Forecast", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes)
                    e.Cancel = true;
                else
                    _itemCollection.Delete(new ItemKey(_company.CompanyCode, itemNumber));
            }
            catch (Exception ex)
            {
                Utility.GetInstance().HandleException(this, ex, e);
