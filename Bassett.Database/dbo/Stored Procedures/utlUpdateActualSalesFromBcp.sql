...
BEGIN TRY
	SELECT	company_code, customer_number, LEFT(item_number, 12) item_number, CAST([month] + '-01-' + [year] AS datetime) [month], SUM(CAST(quantity AS int)) quantity
	INTO	#sales
	FROM	ActualSalesBcp
	GROUP BY company_code, customer_number, LEFT(item_number, 12), [year], [month]
