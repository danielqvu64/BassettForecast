...
BEGIN TRY
	SELECT	co.organization_code company_code, p.customer_number, left(p.item_number, 12) item_number, w.week_end_date, SUM(CAST(p.quantity as int)) quantity
	INTO	#newPOS
	FROM	POSBcp p
	JOIN	WeekCalendar w
	...
	GROUP BY co.organization_code, p.customer_number, left(p.item_number, 12), w.week_end_date
	ORDER BY 1, 2, 3, 4
